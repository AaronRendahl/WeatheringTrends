...
$ln(y_i) = m(x_i) + \epsilon_i$, where $\epsilon_i \sim N(0, s^2(x_i))$

$$
m(x) =
\begin{cases}
  r-c \text{, for } x < pd\\
  ln\left(e^r - (e^r - e^{r-c}) \frac{d - x}{d - pd}\right) \text{, for } pd \geq x > d\\
  r \text{, for } x \geq d
\end{cases}
$$
...
* $r$ is the value of $ln(y)$ at the deepest slope break where  
* $y = e^r$ and $r$ = mobile:immobile  
* $r-c$ is the difference in $ln(y)$ between the two slope breaks  
* $s_1$ and $s_2$ are the standard deviations of $ln(y)$ before the first slope break and after the second slope break, respectively
